...
$BSorgu = $db->prepare("SELECT * FROM video_galeri WHERE durum = ? AND dil = ? ORDER BY sira ASC LIMIT $show,$limit");
    ...
    <div class="vk-breadcrumb">
        <nav class="container">
            <ul>
				...
				<?php }?>
                <li class="active"><?=@$dil['txt85'];?></li>
            </ul>
        </nav>
    </div>
    <!--./vk-breadcrumb-->
    <div class="vk-page vk-page-about" id="foto_galeri">
        <div class="vk-who-we-are vk-section vk-section-style-1">
            <div class="container">
				...
				<div class="row clearfix">
					...
					<!-- Blog -->
					<div class="col-lg-<?php echo $limitayar['limit_video'];?> col-md-<?php echo $limitayar['limit_video'];?> blog-col">
						<div class="card card-active">
							<span class="iconset"><i class="fab fa-youtube"></i></span>
							<div class="blog-img">
								<a href="<?php echo $htc['videodetayurl']; ?>/<?php echo $BSonuc['seo']; ?><?php echo $html;?>">
									<img class="w-100" src="<?php echo tema;?>/uploads/videogaleri/kapak/<?php echo $BSonuc['resim']; ?>" alt="<?php echo $BSonuc['adi']; ?>" />
								</a>
							</div>
							<div class="blog-body">
								<div class="blog-content">
									<h5 class="blog-title">
										<a href="<?php echo $htc['videodetayurl']; ?>/<?php echo $BSonuc['seo']; ?><?php echo $html;?>"><?php echo $BSonuc['adi']; ?></a>
									</h5>
								</div>
							</div>
						</div>
				...
				<nav class="box-pagination text-center">
					<p class="text-center"><?php echo $total;?> <?=@$dil['txt49'];?>  <?php echo $page;?> - <?php echo $limitayar['limit_sayfavideo'];?> <?=@$dil['txt50'];?></p>
					<ul class="vk-pagination">
						<?php
						if($limitayar['limit_sayfavideo'] < $total && $limitayar['limit_sayfavideo'] > 0){
						$showing = 3;
						if($page > 1){?>
						<?php $previous = $page - 1;?>
						<li class="back arrow"><a href="<?php echo $htc['videourl'];?>/<?php echo $previous;?><?php echo $html;?>"><?=@$dil['txt24'];?></a></li>
						<?php }
						...
						if($page != $page_count){?>
						<?php  $next = $page +1;?>
						<li class="next arrow"><a href="<?php echo $htc['videourl'];?>/<?php echo $next; ?><?php echo $html;?>"><?=@$dil['txt25'];?></a></li>
						<?php }} ?>
					</ul>
				</nav>
				<?php }?>
